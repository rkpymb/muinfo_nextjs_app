...
const YourComponent = () => {
    const [data, setData] = useState([]); // Array jo API se aata hai
    const [currentIndex, setCurrentIndex] = useState(0); // Current index ko track karne ke liye
    ...
    const GetData = async () => {
        ...
        const sendUM = { dataid };
        try {
            const response = await fetch("/api/V3/List/MainSliderList", {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                ...
                console.log(parsed.ReqD.MainSliderlist);
                setData(parsed.ReqD.MainSliderlist);
                setIsLoading(false);
            } else {
    ...
    }, [data]); // useEffect tab chalega jab data array change hoga

    return (
        <div className={Mstyles.HBoxCont}>
            <div className={Mstyles.HBoxOverlayBg}>
                <div className={Mstyles.AnimIconABox}>
                    <div className={Mstyles.AnimIconA}>
            ...
            <div className={Mstyles.HBoxOverlay}>
                <div className={Mstyles.HBox}>
                    <div className={Mstyles.HBoxA}>
                        {!isLoading &&
                            <div className={Mstyles.wrapN}>
                                <div className={Mstyles.circleN}>
                                    <div>
                                        <Image
                                            src={`${MediaFilesUrl}${MediaFilesFolder}/${data[currentIndex].image}`}
                                            alt="image"
                                            placeholder="blur"
                                            ...
                                            width={0}
                                            height={0}
                                            className={`${animate ? Mstyles.AnimateTopimg : Mstyles.AnimateTopimgTwo}`}
                                        />
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className={Mstyles.HBoxB}>
                        {data.length > 0 ? <div>
                            <div className={`${animate ? Mstyles.Animatedtitle : Mstyles.AnimatedtitleTwo}`}>
                                <h1> {data[currentIndex].MainText}</h1>
                            </div>
                        </div > : <div>
                        ...
                        <div>
                            {data.length > 0 ? <span className="homeHeading__subtitle">
                                Search over 73,000 local professionals with reviews, pricing, and
                                more.
                            </span> : <div>
                                <div style={{ height: 20 }}></div>
                                <Skeleton variant="text" sx={{ fontSize: '0.1rem', width: 400 }} />
